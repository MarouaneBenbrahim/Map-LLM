...
class ScenarioController:
    ...
    def __init__(self, integrated_system, load_model, power_grid, sumo_manager=None):
        ...
        # Scenario parameters
        self.current_time = 12.0  # Hour (0-24)
        self.current_temperature = 72  # Fahrenheit
        self.time_speed = 1.0  # Simulation speed multiplier
        self.auto_time_advance = False
    ...
    def set_time(self, hour: float):
        """Set simulation time (0-24)"""
        self.current_time = hour % 24
        self.load_model.set_time_of_day(self.current_time)
        self._update_all_loads()

        self._log_event("TIME_CHANGE", f"Time set to {int(hour):02d}:00")
        ...
        while self.monitoring:
            try:
                ...
                if self.auto_time_advance:
                    self.current_time = (self.current_time + 0.1 * self.time_speed) % 24
                    self.load_model.set_time_of_day(self.current_time)
    ...
    def get_load_forecast(self, hours_ahead: int = 6) -> List[Dict[str, Any]]:
        ...
        for h in range(hours_ahead):
            future_time = (self.current_time + h) % 24
